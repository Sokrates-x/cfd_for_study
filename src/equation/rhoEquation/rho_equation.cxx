...
Rho_equation::Rho_equation(const Mesh &m, const Boundary_condtions &bc, Fluid &f,
    Fluid &f_old, const double &r, Time &t) : Equation(m, bc, f, f_old, r, t) {

    for(auto m : *(fluid_.rho())){
    ...
    while(iter_A != A_.end() && iter_y != y_.end() && iter_n != mesh_.vec_node_p().end()){
        ...
        if((*iter_n)->m_x_boundary()){
            ...
            case Boundary_condtions::type::calculated :
                ...
                A_t_n = dx/dt*3/8;
                A_u_n = ((u_p_now >= -u_n_now) ? (u_p_now + u_n_now)/2 : 0) - u_n_now;

                *((*iter_A).begin() + n_x_diff) = A_t_n + A_u_n;

                A_t_p = dx/dt/8;
                A_u_p = ((u_p_now < -u_n_now) ? (u_p_now + u_n_now)/2 : 0);

                *((*iter_A).begin() + p_x_diff) = A_t_p + A_u_p;

                *iter_y = (rho_n_old*3 + rho_p_old)*dx/dt/8;
            ...
            case Boundary_condtions::type::calculated :
                ...
                A_t_n = dx/dt*3/8;
                A_u_n = ((u_m_now <= -u_n_now) ? -(u_m_now + u_n_now)/2 : 0) + u_n_now;

                *((*iter_A).begin() + n_x_diff) = A_t_n + A_u_n;

                A_t_m = dx/dt/8;
                A_u_m = ((u_m_now > -u_n_now) ? -(u_m_now + u_n_now)/2 : 0);

                *((*iter_A).begin() + m_x_diff) = A_t_m + A_u_m;

                *iter_y = (rho_n_old*3 + rho_m_old)*dx/dt/8;
            ...
            A_u_n = ((u_n_now >= -u_p_now) ? ((u_p_now + u_n_now)/2) : 0)
                    - ((u_m_now <= -u_n_now) ? ((u_m_now + u_n_now)/2) : 0);
            ...
            *((*iter_A).begin() + m_x_diff) = A_t_m + A_u_m;
            ...
            A_u_p = ((u_n_now < -u_p_now) ? ((u_p_now + u_n_now)/2) : 0);

            *((*iter_A).begin() + p_x_diff) = A_t_p + A_u_p;

            *iter_y = rho_n_old*dx/dt;
        }
...
void Rho_equation::correct(){

    Equation::correct(fluid_.rho()->begin(), fluid_.rho()->end());

    fluid_.correct(t_rho_tag());
}


void Rho_equation::solve(){
